...
class ComplexFloat(ctypes.Structure):
  ...
  def __init__(self, real_value=0, imag_value=0):
    """Constructs a complex number from two real, or one complex, value(s)."""
    real = c_float()
    imag = c_float()
    if type(real_value) is ComplexFloat or type(real_value) is ComplexDouble:
...
class BlasMatrixView(object):
  ...
  class ViewInt(ctypes.Structure):
    ...
    _fields_ = [('height', catamari_int), ('width', catamari_int),
                ('leading_dim', catamari_int), ('data', POINTER(catamari_int))]
  ...
  class ViewComplexFloat(ctypes.Structure):
    ...
    _fields_ = [('height', catamari_int), ('width', catamari_int),
                ('leading_dim', catamari_int), ('data', POINTER(ComplexFloat))]

  class ViewComplexDouble(ctypes.Structure):
    """An equivalent of CatamariBlasMatrixViewComplexDouble."""
    _fields_ = [('height', catamari_int), ('width', catamari_int),
                ('leading_dim', catamari_int), ('data', POINTER(ComplexDouble))]
  ...
  def from_numpy(cls, matrix):
    ...
    if len(matrix.shape) != 2:
      raise ValueError('Expected a matrix input for BlasMatrixView.from_numpy')
    mat_f = matrix if matrix.flags.f_contiguous else np.asfortranarray(matrix)
...
def SampleLowerHermitianDPP(block_size, maximum_likelihood, matrix_view,
    use_openmp=False):
  """Samples a Hermitian DPP from its marginal kernel.

  Args:
    block_size (catamari_int): The algorithmic block size of the factorization.
  ...
  if use_openmp and have_openmp:
    if matrix_view.dtype == np.float32:
      lib.CatamariOpenMPSampleLowerHermitianDPPFloat.argtypes = [
          catamari_int, c_bool, POINTER(BlasMatrixView.ViewFloat),
          POINTER(BufferInt)]
    ...
    if matrix_view.dtype == np.float32:
      ...
      lib.CatamariSampleLowerHermitianDPPDouble.argtypes = [
          catamari_int, c_bool, POINTER(BlasMatrixView.ViewDouble),
          POINTER(BufferInt)]
  ...
  sample_memory_view = memory_view_from_memory(
      sample.data, ctypes.sizeof(catamari_int)*sample.size)
  return np.frombuffer(sample_memory_view, catamari_int).copy()


def SampleNonHermitianDPP(block_size, maximum_likelihood, matrix_view,
    use_openmp=False):
  ...
  if use_openmp and have_openmp:
    if matrix_view.dtype == np.float32:
      ...
      lib.CatamariOpenMPSampleNonHermitianDPPComplexFloat.argtypes = [
          catamari_int, c_bool, POINTER(BlasMatrixView.ViewComplexFloat),
          POINTER(BufferInt)]
      ...
      lib.CatamariOpenMPSampleNonHermitianDPPComplexDouble.argtypes = [
          catamari_int, c_bool, POINTER(BlasMatrixView.ViewComplexDouble),
          POINTER(BufferInt)]
...
def DPPLogLikelihood(matrix_view):
  ...
  if matrix_view.dtype == np.float32:
    ...
    lib.CatamariDPPLogLikelihoodFloat.argtypes = [
        POINTER(BlasMatrixView.ViewFloat), POINTER(c_float)]
    ...
    lib.CatamariDPPLogLikelihoodDouble.argtypes = [
        POINTER(BlasMatrixView.ViewDouble), POINTER(c_double)]
    ...
    lib.CatamariDPPLogLikelihoodComplexFloat.argtypes = [
        POINTER(BlasMatrixView.ViewComplexFloat), POINTER(c_float)]
    ...
    lib.CatamariDPPLogLikelihoodComplexDouble.argtypes = [
        POINTER(BlasMatrixView.ViewComplexDouble), POINTER(c_double)]
...
if __name__ == '__main__':
  ...
  sample = SampleLowerHermitianDPP(128, False, identity_view, use_openmp=False)
